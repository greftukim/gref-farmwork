...
const SHIFTS = {
  day: { l: '주간', tone: 'primary', c: T.primary },
  night: { l: '야간', tone: 'info', c: T.info },
  off: { l: '휴무', tone: 'muted', c: T.mutedSoft },
};
    ...
    <div style={{ flex: 1, overflow: 'auto', background: T.bg, minWidth: 0 }}>
      ...
      <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {['day', 'night', 'off'].map((k) => {
            const cfg = SHIFTS[k];
            const list = byShift[k] || [];
            return (
              <Card key={k} pad={0} style={{ overflow: 'hidden' }}>
                <div style={{ padding: '14px 16px', borderBottom: `1px solid ${T.borderSoft}`, background: T.bg, borderTop: `3px solid ${cfg.c}` }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: T.text }}>{cfg.l}</span>
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: cfg.c + '22', color: cfg.c }}>{list.length}명</span>
                  </div>
                </div>
