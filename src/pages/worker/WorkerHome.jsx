...
export default function WorkerHome() {
  ...
  const [fabTab, setFabTab] = useState('call'); // 'call' | 'issue'
  ...
  const [issuePhoto, setIssuePhoto] = useState(null); // { file, url }
  ...
  const handleIssueSubmit = async () => {
    if (!issueCategory || !issueForm.zoneId || !issueForm.comment.trim()) return;
    if (issueSubmitting) return;
    setIssueSubmitting(true);
    await addIssue({
      workerId: currentUser.id,
      zoneId: issueForm.zoneId,
      type: issueCategory,
  ...
  return (
    <div className="pb-4">
      ...
      {/* FAB 바텀시트 */}
      <BottomSheet isOpen={showFab} onClose={() => setShowFab(false)} title="신고 · 호출">
        {/* 탭 */}
        ...
        {fabTab === 'issue' && (
          <div className="space-y-3">
            ...
            <Button
              ...
              disabled={issueSubmitting || !issueCategory || !issueForm.zoneId || !issueForm.comment.trim()}
            >
