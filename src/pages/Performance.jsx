...
// 데이터 소스: harvest_records + employees (DB 직접 연결)
...
const PerfHeader = ({ subtitle, title, level, period, setPeriod, crop, setCrop, showBottom, setShowBottom, branchFilter, setBranchFilter, extra }) => (
  <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}` }}>
    ...
    <div style={{ padding: '8px 32px', background: T.bg, borderTop: `1px solid ${T.borderSoft}`, fontSize: 11, color: T.mutedSoft, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span><strong style={{ color: T.text }}>정규화 기준:</strong> 수확 실적 평균 대비 <strong>수확 성과율 %</strong> · 주간 수확량 kg/주 · 지점·작물 차이를 흡수하여 공정하게 비교</span>
    </div>
  </div>
);
            ...
            <div key={w.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 8px', borderRadius: 6, background: i === 0 ? T.bg : 'transparent' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: T.mutedSoft, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Dot c={w.bc} /> {BRANCHES.find(b => b.id === w.branch)?.name} · {w.crop}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: toneByPct(typeof v === 'number' && unit === '%' ? v : 100), fontVariantNumeric: 'tabular-nums', letterSpacing: -0.3 }}>
                  {typeof v === 'number' && v % 1 !== 0 ? v.toFixed(1) : (typeof v === 'number' ? v.toLocaleString() : v)}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {sorted.map(w => (
          <div key={w.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 0' }}>
            ...
            <span style={{ fontSize: 10, color: T.mutedSoft }}>{BRANCHES.find(b => b.id === w.branch)?.name}</span>
...
const PerfKPIs = ({ workers }) => {
  ...
  const avgEff = avgHarv;
  const above100 = workers.filter(w => w.harvestPct >= 100).length;
  const avgAtt = Math.round(workers.reduce((s, w) => s + w.attendance, 0) / workers.length);

  const items = [
    { l: '평균 수확 성과율', v: avgEff, u: '%', sub: '평균 대비 %', tone: toneByPct(avgEff) },
    { l: '평균 수확 달성률', v: avgHarv, u: '%', sub: '전체 평균 기준', tone: toneByPct(avgHarv) },
  ...
  return (
    <Card>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0 }}>작물·작업별 표준공수(SAM) (주/분)</h3>
        <span style={{ fontSize: 11, color: T.mutedSoft }}>DB 적재 기준값</span>
      </div>
      <div style={{ fontSize: 11, color: T.mutedSoft, marginBottom: 14 }}>분/주 기준 표준공수 — 실측 연동은 작업 이력 DB 연결 후 가능</div>
                      ...
                      {std ? (
                        <>
                          <div style={{ fontSize: 15, fontWeight: 700, color: T.text, letterSpacing: -0.3, fontVariantNumeric: 'tabular-nums' }}>
                            {std.toFixed(2)}<span style={{ fontSize: 10, color: T.mutedSoft, fontWeight: 500, marginLeft: 2 }}>분/주</span>
                          </div>
                          <div style={{ fontSize: 10, fontWeight: 600, color: T.mutedSoft, marginTop: 2 }}>{(1 / std).toFixed(1)}주/분</div>
                        </>
                      ) : (
  ...
  return (
    <Card pad={0}>
      <div style={{ padding: '12px 16px', borderBottom: `1px solid ${T.borderSoft}`, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 11, color: T.mutedSoft }}>{workers.length}명 · 수확 성과율 기준 정렬</span>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: T.bg, borderBottom: `1px solid ${T.border}` }}>
            ...
            <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: T.muted }}>수확 성과율</th>
            <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: T.muted }}>수확 달성률</th>
            <th style={{ padding: '10px 12px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: T.muted }}>주간 수확량 (kg)</th>
            ...
            return (
              <tr key={w.id} style={{
                ...
                <td style={{ padding: '10px 12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: T.muted }}>
                    <Dot c={w.bc} />
                    <span style={{ fontWeight: 600, color: T.text }}>{BRANCHES.find(b => b.id === w.branch)?.name}</span>
                    <span>·</span>
                    <span>{w.crop}</span>
                  </div>
                </td>
                <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: toneByPct(w.harvestPct), fontVariantNumeric: 'tabular-nums', letterSpacing: -0.3 }}>
                ...
                <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: T.text, fontVariantNumeric: 'tabular-nums' }}>
                    {w.stemsWeek.toLocaleString()}<span style={{ fontSize: 10, color: T.mutedSoft, fontWeight: 500, marginLeft: 2 }}>kg</span>
                  </div>
                  <div style={{ fontSize: 9, color: T.mutedSoft }}>{w.crop} 주간 평균</div>
                </td>
  ...
  return (
    <Card style={{ borderLeft: `3px solid ${T.danger}`, background: T.dangerSoft, display: 'flex', alignItems: 'center', gap: 12 }} pad={14}>
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>2주 이상 성과율 80% 미만 또는 출근률 저조: {alerts.map(w => w.name).join(', ')}</div>
      </div>
...
function HQPerformanceScreen() {
  ...
  const byEff = [...workers].sort((a, b) => b.harvestPct - a.harvestPct);
  const byHarv = [...workers].sort((a, b) => b.harvestPct - a.harvestPct);
  const bySpeed = [...workers].sort((a, b) => b.stemsWeek - a.stemsWeek);
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: T.bg, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          <TopFiveCard title="수확 성과율 Top 5" subtitle="평균 대비 %" metric="harvestPct" unit="%" items={byEff} target={100} emphasis footer="전체 작업자 평균 수확량 대비 개인 달성 비율. 지점·작물 차이를 흡수하여 공정하게 비교 가능." />
          <TopFiveCard title="수확 달성률 Top 5" subtitle="전체 평균 기준" metric="harvestPct" unit="%" items={byHarv} target={100} footer="수확 실적 기반. 평균=100% 기준으로 정규화." />
          <TopFiveCard title="주간 수확량 Top 5" subtitle="kg/주" metric="stemsWeek" unit="kg/주" items={bySpeed} target="평균 기준" footer="최근 30일 평균 주간 수확량 (kg). 작물별 단위 차이 주의." />
        </div>

        {showBottom && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            <BottomFiveCard items={workers} metric="harvestPct" unit="%" subtitle="성과율 80% 미만" />
            <BottomFiveCard items={workers} metric="stemsWeek" unit="kg/주" subtitle="주간 수확량 낮음" />
        ...
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0 }}>지점별 성과 비교</h3>
            <span style={{ fontSize: 11, color: T.mutedSoft }}>수확 성과율 · 출근률</span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${T.border}` }}>
                ...
                <th style={{ padding: '8px 12px', textAlign: 'right', fontSize: 10, color: T.mutedSoft, fontWeight: 700 }}>평균 성과율</th>
            ...
            <tbody>
              {BRANCHES.map(b => {
                const ws = allWorkers.filter(w => w.branch === b.id);
                ...
                const total = Math.round(avgHarv * 0.85 + avgAtt * 0.15);
...
function BranchPerformanceScreen() {
  ...
  let workers = allWorkers.filter(w => w.branch === 'busan');
  if (crop !== '전체') workers = workers.filter(w => w.crop === crop);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: T.bg, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          <TopFiveCard title="수확 성과율 Top 5" subtitle="평균 대비 %" metric="harvestPct" unit="%" items={byEff} target={100} emphasis />
          <TopFiveCard title="수확 달성률 Top 5" subtitle="전체 평균 기준" metric="harvestPct" unit="%" items={byHarv} target={100} />
          <TopFiveCard title="주간 수확량 Top 5" subtitle="kg/주" metric="stemsWeek" unit="kg/주" items={bySpeed} target="평균 기준" />
        </div>

        {showBottom && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            <BottomFiveCard items={workers} metric="harvestPct" unit="%" subtitle="성과율 낮은 순" />
            <BottomFiveCard items={workers} metric="stemsWeek" unit="kg/주" subtitle="주간 수확량 낮음" />
            <BottomFiveCard items={workers} metric="attendance" unit="%" subtitle="출근률 저조" />
          </div>
        )}
...
function PerformanceDetailScreen() {
  ...
  const weeks = w.weeklyTrend && w.weeklyTrend.length > 0 ? w.weeklyTrend : [
    ...
    { w: '15주', eff: 100, harv: 95 }, { w: '16주', eff: 102, harv: 97 }, { w: '17주', eff: 105, harv: 99 },
  ];

  const workTypes = [
    { t: '정식', eff: Math.round(w.harvestPct * 0.96) },
    { t: '유인', eff: Math.round(w.harvestPct * 0.87) },
    { t: '적엽', eff: Math.round(w.harvestPct * 0.92) },
    { t: '적과', eff: Math.round(w.harvestPct * 0.77) },
    { t: '수확', eff: Math.round(w.harvestPct * 1.03) },
    { t: '선별', eff: Math.round(w.harvestPct * 0.80) },
  ];
  const maxScore = 140;
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ background: T.surface, borderBottom: `1px solid ${T.border}`, padding: '20px 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: T.mutedSoft, marginBottom: 10 }}>
          ...
          <span style={{ cursor: 'pointer' }}>{BRANCHES.find(b => b.id === w.branch)?.name}</span><span>›</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              ...
              <Pill tone="primary">{w.role}</Pill>
              {w.pinned && <Pill tone="warning">우수 작업자</Pill>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, fontSize: 12, color: T.muted }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Dot c={w.bc} />{BRANCHES.find(b => b.id === w.branch)?.name} · {w.crop}</span>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: T.bg, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            { l: '수확 성과율', v: w.harvestPct, u: '%', sub: '전체 평균 대비', tone: toneByPct(w.harvestPct) },
            { l: '수확 달성률', v: w.harvestPct, u: '%', sub: '담당 작물 기준', tone: toneByPct(w.harvestPct) },
            { l: '주간 수확량', v: w.stemsWeek.toLocaleString(), u: 'kg/주', sub: `출근일 ${Math.round(w.attendance / 100 * 30)}일 누적`, tone: T.text },
            { l: '출근률', v: w.attendance, u: '%', sub: `기록일 ${w.weeklyTrend?.length || 0}주`, tone: T.text },
          ].map((k, i) => (
            <Card key={i} pad={18}>
              <div style={{ fontSize: 11, color: T.muted, fontWeight: 600 }}>{k.l}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 16 }}>
          <Card>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
              <div>
                <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0 }}>주별 수확 추이</h3>
                <div style={{ fontSize: 11, color: T.mutedSoft, marginTop: 2 }}>
                  성과율 % · 달성률 % · 기준 100% (본인 평균)
                  {weekWork !== '전체' && <span style={{ marginLeft: 6, color: T.primaryText, fontWeight: 600 }}>· {weekWork} 작업만</span>}
                </div>
              </div>
              <div style={{ display: 'flex', gap: 12, fontSize: 11 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 10, height: 2, background: T.primary }} /><span style={{ color: T.muted }}>성과율</span></span>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 10, height: 2, background: T.success }} /><span style={{ color: T.muted }}>달성률</span></span>
              </div>
            </div>
            ...
            <div style={{ height: 180 }}>
              <svg viewBox="0 0 440 180" width="100%" height="180" preserveAspectRatio="none">
                ...
                <text x="10" y="86" fontSize="9" fill={T.warning} fontWeight="700">기준 100%</text>
                <polyline points={weeks.map((wk, i) => `${i * (440 / Math.max(weeks.length - 1, 1))},${180 - Math.max(0, Math.min(180, (wk.eff - 70) * 3))}`).join(' ')} fill="none" stroke={T.primary} strokeWidth="2.5" />
                {weeks.map((wk, i) => (
                  <g key={i}>
                    <circle cx={i * (440 / Math.max(weeks.length - 1, 1))} cy={180 - Math.max(0, Math.min(180, (wk.eff - 70) * 3))} r="4" fill={T.primary} stroke="#fff" strokeWidth="2" />
                    <text x={i * (440 / Math.max(weeks.length - 1, 1))} y={180 - Math.max(0, Math.min(180, (wk.eff - 70) * 3)) - 10} fontSize="9" fill={T.text} fontWeight="700" textAnchor="middle">{wk.eff}%</text>
                  </g>
                ))}
                <polyline points={weeks.map((wk, i) => `${i * (440 / Math.max(weeks.length - 1, 1))},${180 - Math.max(0, Math.min(180, (wk.harv - 70) * 3))}`).join(' ')} fill="none" stroke={T.success} strokeWidth="2" strokeDasharray="4 2" />
                {weeks.map((wk, i) => <circle key={i} cx={i * (440 / Math.max(weeks.length - 1, 1))} cy={180 - Math.max(0, Math.min(180, (wk.harv - 70) * 3))} r="3" fill={T.success} stroke="#fff" strokeWidth="1.5" />)}
              </svg>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: T.mutedSoft, marginTop: 6 }}>
              {weeks.map(wk => <span key={wk.w}>{wk.w}</span>)}
            </div>
          </Card>

          <Card>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0, marginBottom: 4 }}>작업유형별 효율</h3>
            <div style={{ fontSize: 11, color: T.mutedSoft, marginBottom: 10 }}>{w.crop} · 수확 성과율 기준 추정</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 16 }}>
          <Card>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0, marginBottom: 4 }}>주간 수확량 추이 (kg)</h3>
            <div style={{ fontSize: 11, color: T.mutedSoft, marginBottom: 12 }}>{w.crop} · 수확 실적 (harvest_records 기준)</div>
            <div style={{ height: 150 }}>
              <svg viewBox="0 0 440 150" width="100%" height="150" preserveAspectRatio="none">
                ...
                {weeks.length > 1 && (
                  <polyline
                    points={weeks.map((wk, i) => {
                      const x = i * (440 / Math.max(weeks.length - 1, 1));
                      const maxKg = Math.max(...weeks.map(w2 => w2.kg || 0), 1);
                      const y = 150 - ((wk.kg || 0) / maxKg) * 130;
                      return `${x},${y}`;
                    }).join(' ')}
            ...
            <div style={{ display: 'flex', gap: 14, fontSize: 11, marginTop: 8, justifyContent: 'center' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><span style={{ width: 10, height: 2, background: T.primary }} /><span style={{ color: T.muted }}>주간 수확량 (kg)</span></span>
            </div>
          </Card>

          <Card pad={0}>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${T.borderSoft}`, display: 'flex', justifyContent: 'space-between' }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0 }}>주간 수확 현황</h3>
              <span style={{ fontSize: 11, color: T.mutedSoft }}>harvest_records 집계</span>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <tbody>
                {weeks.slice(0, 5).map((wk, i) => (
                  <tr key={i} style={{ borderBottom: `1px solid ${T.borderSoft}` }}>
                    <td style={{ padding: '8px 16px', fontSize: 11, color: T.muted, fontVariantNumeric: 'tabular-nums' }}>{wk.w}</td>
                    <td style={{ padding: '8px 0', fontSize: 12, fontWeight: 600, color: T.text }}>수확</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', fontSize: 11, color: T.muted, fontVariantNumeric: 'tabular-nums' }}>{(wk.kg || 0).toLocaleString()} kg</td>
                    <td style={{ padding: '8px 16px', textAlign: 'right', fontSize: 11, fontWeight: 700, color: toneByPct(wk.eff || 100), fontVariantNumeric: 'tabular-nums' }}>{wk.eff || 100}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Card>
          ...
          <div style={{ padding: 12, background: T.bg, borderRadius: 8, fontSize: 12, color: T.muted }}>
            코멘트 기능은 준비 중입니다.
          </div>
        </Card>
      </div>
...
function PerformanceCompareScreen() {
  ...
  const picks = [...allWorkers].sort((a, b) => b.harvestPct - a.harvestPct).slice(0, 4);

  const rows = [
    { l: '지점 · 작물', get: w => `${BRANCHES.find(b => b.id === w.branch)?.name || w.branch} · ${w.crop}` },
    { l: '수확 성과율 (%)', get: w => w.harvestPct, tone: w => toneByPct(w.harvestPct), big: true, suffix: '%' },
    { l: '수확 달성률 (%)', get: w => w.harvestPct, tone: w => toneByPct(w.harvestPct), big: true, suffix: '%' },
    { l: '주간 수확량 (kg/주)', get: w => w.stemsWeek.toLocaleString(), big: true },
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: T.bg, display: 'flex', flexDirection: 'column', gap: 16 }}>
        <Card style={{ background: T.primarySoft, borderLeft: `3px solid ${T.primary}`, fontSize: 12, color: T.primaryText, lineHeight: 1.6 }} pad={14}>
          <strong>정규화 비교:</strong> 선택된 {picks.length}명은 지점·작물이 서로 다를 수 있습니다. <strong>수확 성과율 %</strong>와 <strong>수확 달성률 %</strong>가 공정한 비교 지표입니다.
        </Card>
          ...
          {picks.map(w => (
            <Card key={w.id} pad={16} style={{ textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, fontSize: 11, color: T.mutedSoft, marginTop: 2 }}>
                <Dot c={w.bc} />{BRANCHES.find(b => b.id === w.branch)?.name || w.branch}
              </div>
        ...
        <Card>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0, marginBottom: 16 }}>수확 성과율 비교 (정규화)</h3>
            ...
            {picks.map(w => (
              <div key={w.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, height: 24, background: T.bg, borderRadius: 6, overflow: 'hidden', position: 'relative' }}>
                  <div style={{
                    width: `${Math.min(w.harvestPct / 140 * 100, 100)}%`,
                    height: '100%',
                    background: toneByPct(w.harvestPct),
