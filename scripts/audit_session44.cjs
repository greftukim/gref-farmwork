/**
 * 세션 44 — WARN 0 달성 (E-4 farm_admin 필터 실증) + BRANCH-WORK-SCHEDULE-UI-001
 *           + 세션 43 누적 회귀 방어 (97건 → 확장)
 * 실행: node scripts/audit_session44.cjs
 */
...
const OUT_DIR = path.join(__dirname, '..', 'docs', 'regression_session44');
...
(async () => {
  console.log('=== 세션 44 검증 + 세션 43 누적 회귀 방어 감사 ===\n');
  ...
  // ════════════════════════════════════════════
  // SECTION J: BRANCH-WORK-SCHEDULE-UI-001 (신규 — 세션 44)
  // ════════════════════════════════════════════
  console.log('\n[SECTION J] BRANCH-WORK-SCHEDULE-UI-001 근무시간 설정 UI 검증');
  ...
  console.log(`스크린샷: docs/regression_session44/`);
