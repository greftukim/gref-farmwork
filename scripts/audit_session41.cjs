/**
 * 세션 41 — STATS-AGGREGATION-001 StatsPage DB 연결 검증
 *          + 세션 40 누적 회귀 방어 (49건)
 * 실행: node scripts/audit_session41.cjs
 *
 * 검증 대상:
 *   Task 1: StatsPage — usePerformanceData 연결, KPI 3개, 랭킹
 *   Task 1: performanceStore 삭제 후 빌드 이상 없음
 *
 * 회귀 방어:
 *   세션 40 — Performance 4화면 (49건 전체 재실행)
 *   세션 39 — GrowthInputScreen, 생육 대시보드
 *   세션 28-38 — 메인 대시보드, 근무 관리, HQ 대시보드
 */
...
const OUT_DIR = path.join(__dirname, '..', 'docs', 'regression_session41');
...
(async () => {
  console.log('=== 세션 41 검증 + 세션 40 누적 회귀 방어 감사 ===\n');
  ...
  console.log('\n[D-2] 근무 관리 타임라인 회귀 (세션 37 SchedulePage 3뷰)');
  ...
  // ════════════════════════════════════════════
  // SECTION E: Stats 성과 분석 페이지 (신규 — 세션 41)
  // ════════════════════════════════════════════
  console.log('\n[SECTION E] Stats 성과 분석 페이지 검증 (STATS-AGGREGATION-001)');
  ...
  statsText.includes('작업자 성과 분석')
    ...
    : log('FAIL', '성과 분석 타이틀 없음 — 화이트 스크린 가능성');

  // 구 "평가 데이터가 없습니다" 패턴이 performanceStore(빈 배열) 원인이었으므로
  // 실데이터가 있으면 이 메시지 안 나와야 함
  statsText.includes('평가 데이터가 없습니다')
    ? log('FAIL', '구 "평가 데이터가 없습니다" 빈 상태 잔존 — performanceStore 제거 미반영')
    : log('PASS', '빈 상태 메시지 없음 (실데이터 로드됨)');

  currentTest = 'STATS-KPI';
  console.log('\n[E-2] KPI 카드 3개 (라벨 + 단위)');
  statsText.includes('평균 수확 성과율')
    ? log('PASS', 'KPI 카드 "평균 수확 성과율" 표시')
    : log('FAIL', '"평균 수확 성과율" KPI 카드 없음');

  statsText.includes('주간 최고 수확량')
    ? log('PASS', 'KPI 카드 "주간 최고 수확량" 표시')
    : log('FAIL', '"주간 최고 수확량" KPI 카드 없음');

  statsText.includes('평가 인원')
    ? log('PASS', 'KPI 카드 "평가 인원" 표시')
    : log('FAIL', '"평가 인원" KPI 카드 없음');

  // 단위 확인
  statsText.includes('kg/주')
    ? log('PASS', '"kg/주" 단위 표시')
    : log('WARN', '"kg/주" 단위 없음');

  statsText.includes('수확 성과 랭킹')
    ? log('PASS', '랭킹 테이블 헤더 "수확 성과 랭킹" 표시')
    : log('FAIL', '"수확 성과 랭킹" 헤더 없음');

  currentTest = 'STATS-RANKING';
  console.log('\n[E-3] 수확 성과 랭킹 — 실 작업자 데이터');
  // harvestPct % 단위가 표시되는지 확인
  const hasPctUnit = await page.locator('text="%"').first().isVisible().catch(() => false);
  hasPctUnit
    ? log('PASS', '랭킹 테이블 % 단위 표시 (harvestPct 연결 확인)')
    : log('WARN', '% 단위 없음');

  // 실 작업자 이름이 있는지 (부산/진주/하동 지점 배지라도)
  const hasBranchBadge = statsText.includes('부산LAB') || statsText.includes('진주HUB') || statsText.includes('하동HUB');
  hasBranchBadge
    ? log('PASS', '지점 배지 표시 (실 직원 데이터 연결 확인)')
    : log('WARN', '지점 배지 없음 — DB 데이터 로드 확인 필요');

  await ss(page, 'stats-ranking');

  currentTest = 'STATS-PERMISSION';
  console.log('\n[E-4] 권한 분기 확인');
  // jhkim은 hr_admin → 전체 지점 표시 (branch 필터 없음)
  // 부산LAB, 진주HUB, 하동HUB 모두 표시되는지 확인
  const allBranches = ['부산LAB', '진주HUB', '하동HUB'].every(b => statsText.includes(b));
  allBranches
    ? log('PASS', 'hr_admin 권한 — 전체 지점 작업자 표시 확인')
    : log('WARN', '지점 일부 미표시 (DB 데이터 분포 확인 필요)');

  // farm_admin 지점 분기는 jhkim(hr_admin)으로 직접 테스트 불가
  // 코드에 branch 필터 로직 존재 여부만 구조적으로 확인
  log('WARN', 'farm_admin 지점 분기: jhkim(hr_admin)으로 테스트 불가 — 코드 분기 존재 확인만 (교훈 66)');
  ...
  console.log(`스크린샷: docs/regression_session41/`);
