/**
 * 세션 46 — HARVEST-TARGETS-001 검증
 *           + 세션 45 누적 회귀 방어 (132건 → 확장)
 * 실행: node scripts/audit_session46.cjs
 */
...
const OUT_DIR = path.join(__dirname, '..', 'docs', 'regression_session46');
...
(async () => {
  console.log('=== 세션 46 검증 + 세션 45 누적 회귀 방어 감사 ===\n');
  ...
  // ════════════════════════════════════════════
  // SECTION K: TASKS-WORKER-ID-MISMATCH-001 (세션 45 회귀)
  // ════════════════════════════════════════════
  ...
  console.log('\n[K-6] farm_admin(hdkim) → /admin/board — 부산 활성 작업 표시');
  ...
  // ════════════════════════════════════════════
  // SECTION L: HARVEST-TARGETS-001 (신규 — 세션 46)
  // ════════════════════════════════════════════
  console.log('\n[SECTION L] HARVEST-TARGETS-001 — 월 수확 목표 KPI 검증 (신규)');
  ...
  const lText1 = await pageL.textContent('body').catch(() => '');
  ...
  (lText2.includes('5000') || lText2.includes('5,000'))
    ...
    : log('WARN', '부산 목표값 미표시 (DB 쿼리 또는 타이밍 확인)');
  ...
  if (l2LoggedIn) {
    ...
    await pageL2.waitForTimeout(3000);
    const lFarmText = await pageL2.textContent('body').catch(() => '');
  ...
  !l5Text.includes('목표 대비 87%') && !l5Text.includes('목표 대비\n87%')
  ...
  console.log(`스크린샷: docs/regression_session46/`);
