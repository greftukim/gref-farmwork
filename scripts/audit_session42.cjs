/**
 * 세션 42 — WORK-STATS-PAGE-001 + BRANCH-STATS-PAGE-001 검증
 *          + 세션 41 누적 회귀 방어 (62건)
 *          + 앱 이름 FarmWork 교체 확인
 * 실행: node scripts/audit_session42.cjs
 *
 * 검증 대상:
 *   Task 1B: WorkStatsPage — is_active 필터 + KPI + 데이터 표시
 *   Task 2:  BranchStatsPage — 3지점 KPI + 비교 섹션
 *   Task 3:  앱 이름 FarmWork 교체
 *
 * 회귀 방어:
 *   세션 41 — StatsPage 62건 전체 재실행
 */
...
const OUT_DIR = path.join(__dirname, '..', 'docs', 'regression_session42');
...
(async () => {
  console.log('=== 세션 42 검증 + 세션 41 누적 회귀 방어 감사 ===\n');
  ...
  log('PASS', 'jhkim(hr_admin) → HQ 성과 페이지 접근 허용 확인');
  ...
  empText.includes('직원 관리') || empText.includes('직원')
    ? log('PASS', '직원 관리 페이지 로드')
    : log('FAIL', '직원 관리 페이지 없음');
  const empRows = await page.locator('table tbody tr').count().catch(() => 0);
  empRows > 0
    ? log('PASS', `직원 목록 ${empRows}행 표시`)
    : log('WARN', '직원 목록 행 없음');
  ...
  // ════════════════════════════════════════════
  // SECTION E: Stats 성과 분석 페이지 (세션 41 회귀)
  // ════════════════════════════════════════════
  console.log('\n[SECTION E] Stats 성과 분석 페이지 검증 (세션 41 회귀)');

  currentTest = 'STATS-LOAD';
  console.log('\n[E-1] /admin/stats — 화이트 스크린 해소 + 타이틀');
  ...
  statsText.includes('작업자 성과 분석')
    ? log('PASS', '성과 분석 타이틀 표시')
    : log('FAIL', '성과 분석 타이틀 없음');

  statsText.includes('평가 데이터가 없습니다')
    ? log('FAIL', '구 빈 상태 잔존 — performanceStore 제거 미반영')
    : log('PASS', '빈 상태 메시지 없음 (실데이터 로드됨)');

  currentTest = 'STATS-KPI';
  console.log('\n[E-2] Stats KPI 카드 3개');
  for (const kpi of ['평균 수확 성과율', '주간 최고 수확량', '평가 인원']) {
    statsText.includes(kpi) ? log('PASS', `KPI 카드 표시: ${kpi}`) : log('FAIL', `KPI 카드 없음: ${kpi}`);
  }
  statsText.includes('kg/주') ? log('PASS', '"kg/주" 단위 표시') : log('WARN', '"kg/주" 단위 없음');
  statsText.includes('수확 성과 랭킹') ? log('PASS', '랭킹 헤더 표시') : log('FAIL', '랭킹 헤더 없음');

  currentTest = 'STATS-RANKING';
  console.log('\n[E-3] 수확 성과 랭킹 — 실 작업자 데이터');
  const hasPctUnit = await page.locator('text="%"').first().isVisible().catch(() => false);
  hasPctUnit ? log('PASS', '랭킹 % 단위 표시') : log('WARN', '% 단위 없음');
  const hasBranchBadge = statsText.includes('부산LAB') || statsText.includes('진주HUB') || statsText.includes('하동HUB');
  hasBranchBadge ? log('PASS', '지점 배지 표시 (실 직원 데이터 연결 확인)') : log('WARN', '지점 배지 없음');
  ...
  allBranches
    ? log('PASS', 'hr_admin — 전체 지점 작업자 표시 확인')
    : log('WARN', '지점 일부 미표시 (harvest_records 없는 지점 가능)');
  log('WARN', 'farm_admin 지점 분기: jhkim(hr_admin)으로 테스트 불가 — 코드 분기 존재 확인만');

  // ════════════════════════════════════════════
  // SECTION F: WorkStatsPage (WORK-STATS-PAGE-001)
  // ════════════════════════════════════════════
  console.log('\n[SECTION F] WorkStatsPage 검증 (WORK-STATS-PAGE-001)');

  currentTest = 'WORK-STATS-LOAD';
  console.log('\n[F-1] /admin/work-stats — 로드 + 타이틀');
  await goto(page, `${BASE_URL}/admin/work-stats`);
  await page.waitForTimeout(1800);
  const wsText = await page.textContent('body').catch(() => '');
  await ss(page, 'work-stats');

  wsText.includes('근무 통계')
    ? log('PASS', '근무 통계 타이틀 표시')
    : log('FAIL', '근무 통계 타이틀 없음');

  wsText.includes('준비 중')
    ? log('FAIL', '"준비 중" stub 잔존 — 재작성 미반영')
    : log('PASS', '"준비 중" stub 제거됨');

  currentTest = 'WORK-STATS-KPI';
  console.log('\n[F-2] KPI 카드 3개 (총 근무 시간 / 총 출근 일수 / 평가 인원)');
  for (const [kpi, unit] of [['총 근무 시간', 'h'], ['총 출근 일수', '일'], ['평가 인원', '명']]) {
    wsText.includes(kpi)
      ? log('PASS', `KPI 카드 표시: ${kpi}`)
      : log('FAIL', `KPI 카드 없음: ${kpi}`);
    wsText.includes(unit)
      ? log('PASS', `단위 표시: ${unit}`)
      : log('WARN', `단위 없음: ${unit}`);
  }

  currentTest = 'WORK-STATS-TABLE';
  console.log('\n[F-3] 직원별 근무 시간 테이블 — 실 데이터');
  wsText.includes('직원별 근무 시간')
    ? log('PASS', '테이블 헤더 "직원별 근무 시간" 표시')
    : log('FAIL', '테이블 헤더 없음');

  // attendance 시드 480+건 → 24명 작업자 데이터 존재해야 함
  // 숫자가 0이 아닌 근무시간(h)이 표시되는지 확인
  const hasNonZeroHours = /[1-9][0-9]*h/.test(wsText);
  hasNonZeroHours
    ? log('PASS', '0h 이상의 근무시간 표시 (attendance 시드 연결 확인)')
    : log('WARN', '근무시간 0h만 표시 — attendance 데이터 로드 확인 필요');

  currentTest = 'WORK-STATS-FILTER';
  console.log('\n[F-4] 기간 필터 입력 요소');
  const dateInputs = await page.locator('input[type="date"]').count().catch(() => 0);
  dateInputs >= 2
    ? log('PASS', `날짜 입력 요소 ${dateInputs}개 (기간 필터 표시)`)
    : log('FAIL', '날짜 입력 요소 없음');

  // ════════════════════════════════════════════
  // SECTION G: BranchStatsPage (BRANCH-STATS-PAGE-001)
  // ════════════════════════════════════════════
  console.log('\n[SECTION G] BranchStatsPage 검증 (BRANCH-STATS-PAGE-001)');

  currentTest = 'BRANCH-STATS-LOAD';
  console.log('\n[G-1] /admin/branch-stats — 로드 + 타이틀');
  await goto(page, `${BASE_URL}/admin/branch-stats`);
  const branchStatsLoaded = await waitForDataLoad(page, 12000);
  branchStatsLoaded ? log('PASS', 'BranchStatsPage 로딩 완료') : log('FAIL', '로딩 타임아웃');
  const bsText = await page.textContent('body').catch(() => '');
  await ss(page, 'branch-stats');

  bsText.includes('지점별 성과')
    ? log('PASS', '지점별 성과 타이틀 표시')
    : log('FAIL', '지점별 성과 타이틀 없음');

  bsText.includes('준비 중')
    ? log('FAIL', '"준비 중" stub 잔존 — 재작성 미반영')
    : log('PASS', '"준비 중" stub 제거됨');
  ...
  console.log('\n[G-2] 3지점 카드 표시 확인');
  for (const branch of ['부산LAB', '진주HUB', '하동HUB']) {
    bsText.includes(branch)
      ? log('PASS', `지점 카드 표시: ${branch}`)
      : log('FAIL', `지점 카드 없음: ${branch}`);
  }

  currentTest = 'BRANCH-STATS-KPI';
  console.log('\n[G-3] KPI 항목 확인 (활성 작업자 / 평균 성과율 / 주간 수확량)');
  bsText.includes('활성 작업자')
    ? log('PASS', '"활성 작업자" 항목 표시')
    : log('FAIL', '"활성 작업자" 항목 없음');
  bsText.includes('평균 성과율')
    ? log('PASS', '"평균 성과율" KPI 표시')
    : log('FAIL', '"평균 성과율" KPI 없음');
  bsText.includes('주간 수확량')
    ? log('PASS', '"주간 수확량" KPI 표시')
    : log('FAIL', '"주간 수확량" KPI 없음');

  currentTest = 'BRANCH-STATS-COMPARE';
  console.log('\n[G-4] 비교 섹션 표시 확인');
  bsText.includes('평균 성과율 비교')
    ? log('PASS', '"평균 성과율 비교" 섹션 표시')
    : log('FAIL', '"평균 성과율 비교" 섹션 없음');
  bsText.includes('주간 수확량 비교')
    ? log('PASS', '"주간 수확량 비교" 섹션 표시')
    : log('FAIL', '"주간 수확량 비교" 섹션 없음');
  await ss(page, 'branch-stats-compare');

  // ════════════════════════════════════════════
  // SECTION H: 앱 이름 FarmWork 교체 (Task 3)
  // ════════════════════════════════════════════
  console.log('\n[SECTION H] 앱 이름 FarmWork 교체 확인');
  await ctx.close();

  // 새 컨텍스트 — 로그인 전 로그인 페이지
  const ctx2 = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const page2 = await ctx2.newPage();

  currentTest = 'APP-NAME-LOGIN';
  console.log('\n[H-1] 로그인 페이지 — "FarmWork" 표시 + 구 명칭 없음');
  await goto(page2, `${BASE_URL}/login`);
  await page2.waitForTimeout(500);
  const loginPageText = await page2.textContent('body').catch(() => '');

  loginPageText.includes('FarmWork')
    ? log('PASS', '"FarmWork" 로그인 페이지 표시')
    : log('FAIL', '"FarmWork" 없음');

  // 구 이름 "GREF FarmWork"가 완전히 사라졌는지 (UI 텍스트에서)
  loginPageText.includes('GREF FarmWork')
    ? log('FAIL', '구 이름 "GREF FarmWork" 잔존')
    : log('PASS', '구 이름 "GREF FarmWork" 제거 확인');

  await ss(page2, 'login-farmwork');

  currentTest = 'APP-NAME-SIDEBAR';
  console.log('\n[H-2] 사이드바 — "FarmWork" 표시');
  // 로그인 후 사이드바 확인
  ...
  sidebarText.includes('FarmWork')
    ? log('PASS', '"FarmWork" 사이드바/UI 표시')
    : log('WARN', '"FarmWork" 사이드바 미확인 (hover 필요 가능)');
  ...
  console.log(`스크린샷: docs/regression_session42/`);
