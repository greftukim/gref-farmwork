/**
 * 세션 49 — HQ-DASHBOARD-INTERACTIVE-001 검증
 *           + 세션 48 누적 회귀 방어 (A-N 전체 유지)
 * 실행: node scripts/audit_session49.cjs
 *
 * 섹션 구성:
 *   A:   로그인
 ...
 *   I:   PROTECTED-ROUTE-001 회귀 (세션 43) — 완전판
 *   J:   BRANCH-WORK-SCHEDULE-UI-001 회귀 (세션 44) — 완전판
 *   K:   TASKS-WORKER-ID-MISMATCH-001 회귀 (세션 45) — 완전판
 *   L:   HARVEST-TARGETS-001 회귀 (세션 46)
 *   M:   DASHBOARD-PHASE2-001 회귀 (세션 47)
 *   N:   HQ-ISSUE-PAGE-001 회귀 (세션 48)
 *   O:   HQ-DASHBOARD-INTERACTIVE-001 신규 (세션 49)
 */
...
const OUT_DIR = path.join(__dirname, '..', 'docs', 'regression_session49');
...
(async () => {
  console.log('=== 세션 49 검증 + 세션 48 누적 회귀 방어 감사 ===\n');
  ...
  // ════════════════════════════════════════════
  // SECTION O: HQ-DASHBOARD-INTERACTIVE-001 신규 (세션 49)
  // ════════════════════════════════════════════
  console.log('\n[SECTION O] HQ-DASHBOARD-INTERACTIVE-001 — 운영 리포트 실데이터 연결 검증 (신규)');

  const ctxO = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const pageO = await ctxO.newPage();
  pageO.on('dialog', async (d) => { await d.accept().catch(() => {}); });
  await loginAs(pageO, 'jhkim');

  currentTest = 'HQ-DASHBOARD-INTERACTIVE';

  console.log('\n[O-1] hr_admin → /admin/hq/interactive — 페이지 로드');
  await goto(pageO, `${BASE_URL}/admin/hq/interactive`);
  await waitForDataLoad(pageO, 25000);
  await pageO.waitForTimeout(2000);
  await ss(pageO, 'hq-interactive-top');
  const oText = await pageO.textContent('body').catch(() => '');
  pageO.url().includes('/admin/hq/interactive')
    ? log('PASS', '/admin/hq/interactive 라우트 도달')
    : log('FAIL', `라우트 미도달 (${pageO.url()})`);
  oText.includes('운영 리포트')
    ? log('PASS', '"운영 리포트" 타이틀 표시 (화이트 스크린 없음)')
    : log('FAIL', '"운영 리포트" 타이틀 미표시');

  console.log('\n[O-2] 실데이터 지점 카드 3개 표시');
  ['부산LAB', '진주HUB', '하동HUB'].forEach((name) => {
    oText.includes(name)
      ? log('PASS', `지점 카드: ${name}`)
      : log('FAIL', `지점 카드 미표시: ${name}`);
  });

  console.log('\n[O-3] KPI 수확량 — 실데이터 (≥10,000kg)');
  // 월 실수확: busan 4148 + jinju 2405 + hadong 4803 = ~11356kg
  const harvestMatch = oText.match(/(\d{1,3}(?:,\d{3})+)\s*kg/);
  const harvestVal = harvestMatch ? parseInt(harvestMatch[1].replace(/,/g, ''), 10) : 0;
  harvestVal >= 10000
    ? log('PASS', `수확량 KPI 실데이터 (${harvestMatch?.[1] || 0}kg ≥ 10,000kg)`)
    : log('FAIL', `수확량 KPI 낮음 또는 미표시 (${harvestVal})`);

  console.log('\n[O-4] KPI 미해결이슈 — 실데이터 (≥6건)');
  oText.includes('미해결 이슈')
    ? log('PASS', '"미해결 이슈" KPI 라벨 표시')
    : log('FAIL', '"미해결 이슈" KPI 미표시');
  // 시드: 8건 미해결
  const issueKpiMatch = oText.match(/미해결 이슈[\s\S]{0,100}(\d+)\s*건/);
  const issueCount = issueKpiMatch ? parseInt(issueKpiMatch[1], 10) : 0;
  issueCount >= 6
    ? log('PASS', `미해결이슈 ${issueCount}건 (≥6 실데이터 확인)`)
    : log('WARN', `미해결이슈 카운트 낮음 (${issueCount})`);

  console.log('\n[O-5] 지점별 수확량 바차트 — 지점명 라벨');
  oText.includes('지점별 수확량 비교')
    ? log('PASS', '"지점별 수확량 비교" 차트 타이틀 표시')
    : log('FAIL', '차트 타이틀 미표시');
  // 막대 클릭 드릴다운 힌트 확인
  oText.includes('막대 클릭')
    ? log('PASS', '"막대 클릭 → 작물별 분해" 힌트 표시')
    : log('WARN', '드릴다운 힌트 미표시');

  console.log('\n[O-6] 드릴다운 — 부산LAB 바 클릭 → 작물별 차트');
  // 바차트에서 부산LAB 클릭
  await pageO.locator('text=부산LAB').first().click().catch(() => {});
  await pageO.waitForTimeout(1000);
  const drillText = await pageO.textContent('body').catch(() => '');
  drillText.includes('작물별 수확량') || drillText.includes('지점 비교로 돌아가기')
    ? log('PASS', '드릴다운 차트 표시 (부산LAB 작물별)')
    : log('WARN', '드릴다운 전환 미확인');
  // 돌아가기
  await pageO.locator('text=지점 비교로 돌아가기').click().catch(() => {});
  await pageO.waitForTimeout(500);

  console.log('\n[O-7] 지점 상세 모달 — 카드 클릭 → 모달 오픈');
  await ss(pageO, 'hq-interactive-cards');
  // 지점 카드 클릭 (3번째 grid item — 하동HUB)
  await pageO.locator('text=하동HUB').first().click().catch(() => {});
  await pageO.waitForTimeout(800);
  const modalText = await pageO.textContent('body').catch(() => '');
  modalText.includes('이번 달 지표') || modalText.includes('작물별 현황')
    ? log('PASS', '지점 상세 모달 오픈 (이번 달 지표 표시)')
    : log('WARN', '모달 오픈 미확인');
  // 모달 닫기
  await pageO.keyboard.press('Escape').catch(() => {});
  await pageO.waitForTimeout(500);

  console.log('\n[O-8] 승인 허브 필터 탭');
  await ss(pageO, 'hq-interactive-mid');
  const oText2 = await pageO.textContent('body').catch(() => '');
  ['전체', '근태', '예산', '인사', '자재'].forEach((tab) => {
    oText2.includes(tab)
      ? log('PASS', `승인 허브 필터 탭: ${tab}`)
      : log('FAIL', `승인 허브 필터 탭 미표시: ${tab}`);
  });

  console.log('\n[O-9] 이상 신고 피드 — 실데이터 (하드코딩 제거 확인)');
  oText2.includes('전 지점 이상 신고')
    ? log('PASS', '"전 지점 이상 신고" 피드 카드 표시')
    : log('FAIL', '"전 지점 이상 신고" 카드 미표시');
  // 시드 데이터 comment 텍스트 확인 (하드코딩 old: '잎 하면 흰색 반점')
  const hasRealComment = oText2.includes('흰가루병') || oText2.includes('진딧물') || oText2.includes('누수') || oText2.includes('황화') || oText2.includes('이슈 없음');
  hasRealComment
    ? log('PASS', '이슈 피드 실데이터 comment 표시 (하드코딩 제거 확인)')
    : log('WARN', '이슈 피드 실데이터 미확인');
  // 구 하드코딩 이름 제거 확인
  !oText2.includes('잎 하면 흰색 반점')
    ? log('PASS', '구 하드코딩 이슈 내용 제거 확인')
    : log('FAIL', '구 하드코딩 이슈 내용 잔존');

  console.log('\n[O-10] 기간 탭 전환 — 수확량 수치 변화 확인');
  const monthHarvestBefore = await pageO.textContent('body').catch(() => '');
  await pageO.locator('button').filter({ hasText: '주' }).first().click().catch(() => {});
  await pageO.waitForTimeout(1000);
  const weekText = await pageO.textContent('body').catch(() => '');
  weekText.includes('주간')
    ? log('PASS', '기간 탭 "주" 전환 — "주간" 라벨 표시')
    : log('WARN', '기간 탭 전환 미확인');
  // 원복
  await pageO.locator('button').filter({ hasText: '월' }).first().click().catch(() => {});
  await pageO.waitForTimeout(500);

  console.log('\n[O-11] 미해결이슈 KPI 클릭 → /admin/hq/issues 이동');
  await goto(pageO, `${BASE_URL}/admin/hq/interactive`);
  await waitForDataLoad(pageO, 20000);
  await pageO.waitForTimeout(1000);
  await pageO.locator('text=미해결 이슈').first().click().catch(() => {});
  await pageO.waitForTimeout(1500);
  await ss(pageO, 'hq-interactive-issue-nav');
  pageO.url().includes('/admin/hq/issues')
    ? log('PASS', '"미해결 이슈" KPI 클릭 → /admin/hq/issues 이동')
    : log('WARN', `이동 미확인 (${pageO.url()})`);

  console.log('\n[O-12] 이슈피드 "전체 →" → /admin/hq/issues 이동');
  await goto(pageO, `${BASE_URL}/admin/hq/interactive`);
  await waitForDataLoad(pageO, 20000);
  await pageO.waitForTimeout(1000);
  // IssueFeedCard의 "전체 →"는 승인허브 "전체 →" 다음 (nth 1)
  await pageO.locator('text=전체 →').nth(1).click().catch(() => {});
  await pageO.waitForTimeout(1500);
  pageO.url().includes('/admin/hq/issues')
    ? log('PASS', '이슈피드 "전체 →" → /admin/hq/issues 이동')
    : log('WARN', `이동 미확인 (${pageO.url()})`);

  console.log('\n[O-13] "지점 관리 →" → /admin/hq/branches 이동');
  await goto(pageO, `${BASE_URL}/admin/hq/interactive`);
  await waitForDataLoad(pageO, 20000);
  await pageO.waitForTimeout(1000);
  await pageO.locator('text=지점 관리 →').click().catch(() => {});
  await pageO.waitForTimeout(1500);
  pageO.url().includes('/admin/hq/branches')
    ? log('PASS', '"지점 관리 →" → /admin/hq/branches 이동')
    : log('WARN', `이동 미확인 (${pageO.url()})`);

  console.log('\n[O-14] farm_admin → /admin/hq/interactive → /admin 리디렉트 (PROTECTED-ROUTE-001)');
  const ctxO2 = await browser.newContext({ viewport: { width: 1440, height: 900 } });
  const pageO2 = await ctxO2.newPage();
  pageO2.on('dialog', async (d) => { await d.accept().catch(() => {}); });
  await loginAs(pageO2, 'hdkim');
  await goto(pageO2, `${BASE_URL}/admin/hq/interactive`);
  await pageO2.waitForTimeout(1500);
  !pageO2.url().includes('/admin/hq')
    ? log('PASS', 'farm_admin → /admin/hq/interactive → /admin 리디렉트')
    : log('FAIL', `farm_admin HQ interactive 접근 허용됨 (${pageO2.url()})`);
  await ctxO2.close();
  ...
  console.log(`스크린샷: docs/regression_session49/`);
  console.log('==================================================');
