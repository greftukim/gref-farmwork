...
| INFRA-001 | INFRA | open | Phase 5 세션 3 (2026-04-11) | 모니터링 (자동 복구 확인) | Supabase PostgREST Warp 서버 'Thread killed by timeout manager' 에러 다수 발생 — HTTP 레이어 타임아웃이 42501(RLS) 에러로 감싸져 반환. INSERT/UPDATE 간헐적 403. SQL Editor 직접 연결 정상. 08:42, 09:54~10:14 발생, 08:55 PostgREST 재시작 확인. **2026-04-11 10:35 KST 반장 PC 재로그인 시 fcm_tokens INSERT 정상 통과 확인(콘솔 'FCM 토큰 저장 완료 (DB)' 로그). 순간 장애로 자동 복구됨. 지속 이슈 아님.** 다음 세션: 재발 여부 확인 → 재발 없으면 mitigated 처리. | docs/BACKLOG.md |
| RLS-DEBT-021 | RLS | open | Phase 5 세션 3 (2026-04-11) | - | safety_checks anon SELECT의 worker 단위 격리 — device_token claim 기반 RLS 재설계 필요. RLS-DEBT-019의 근본 해소 항목. 트랙 F 후보. | docs/BACKLOG.md |
| UX-007 | UX | open | Phase 5 세션 3 (2026-04-11) | - | 알림 권한 denied 상태에서 사용자가 권한 재요청할 수 있는 UI 없음. 시크릿창/처음 사용자에게 권한 안내 + 재시도 버튼 필요. E-6.5 활성화 전 추가. | docs/BACKLOG.md |
