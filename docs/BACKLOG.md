...
| HARVEST-TARGETS-001 | 목표치 설계 | open | 세션 23 (2026-04-23) 발견 → 세션 27 (2026-04-24) 재정의 | - | 지점별 월간 수확 목표치 설계 없음 — Dashboard 달성률/목표 대비 % 계산 불가. branches 테이블 harvest_target_kg 컬럼 ALTER vs 별 harvest_targets 테이블(월별·작물별 세분화) 신설 결정 필요. 박민식·김민국 답변 선행. 구 HARVEST-TABLE-001 rename (harvest_records 0건 이슈는 세션 27 시드 509건 투입으로 해소, 남은 목표치 트랙 분리). | docs/BACKLOG.md |
...
| HQ-APPROVAL-EXPORT-001 | 기능 미구현 | partial | 세션 29 등록 → 세션 30 (2026-04-24) alert 연결 | - | 버튼 무반응 → alert('승인 내역 내보내기 기능 준비 중입니다.') 임시 처리. 실 CSV 내보내기 로직은 미구현 유지. | docs/BACKLOG.md |
...
| HQ-FINANCE-PDF-EXPORT-001 | 기능 미구현 | partial | 세션 29 등록 → 세션 30 (2026-04-24) alert 연결 | - | 버튼 무반응 → alert('재무 PDF 내보내기 기능 준비 중입니다.') 임시 처리. PDF 라이브러리는 HQ-FINANCE-001 실데이터 연결 후 선정 예정. | docs/BACKLOG.md |
...
| HQ-BRANCH-DETAIL-001 | 기능 미구현 | open | 세션 30 (2026-04-24) | - | HQBranchesScreen 지점 카드 "상세 →" 버튼 — alert('지점명 지점 상세 페이지 준비 중입니다.') 임시 처리. `/admin/hq/branches/:branchId` 신규 라우트/페이지 신설 필요. 별 세션. | docs/BACKLOG.md |
...
| FARM-DASH-DATE-001 | 하드코딩 | open | 세션 31 (2026-04-24) | - | AdminDashboard.jsx:231 TopBar title 날짜 하드코딩 "2026년 4월 21일 화요일" — 항상 오표시. new Date().toLocaleDateString('ko-KR') 치환 필요. | docs/AUDIT_SESSION31_FARM.md |
| FARM-DASH-SCHED-HARDCODE-001 | 하드코딩 | open | 세션 31 (2026-04-24) | - | AdminDashboard.jsx:492-493 "이번 주 스케줄" 날짜 배열 + isToday=i===1(화요일 고정) 하드코딩. 현재 주 일자 동적 계산으로 교체 필요. | docs/AUDIT_SESSION31_FARM.md |
| FARM-DASH-BTN-001 | 기능 미구현 | open | 세션 31 (2026-04-24) | - | AdminDashboard TopBar "내보내기"(L232), "새 작업 등록"(L232), AI제안 "적용하기"(L481), "자세히"(L482) — onClick 없음 (패턴 A). alert 임시 처리 또는 기능 구현 필요. | docs/AUDIT_SESSION31_FARM.md |
| FARM-LEAVE-SCOPE-001 | 데이터 스코프 | open | 세션 31 (2026-04-24) | - | leaveStore.fetchRequests() branch 필터 없음 — farm_admin이 전 지점 휴가 신청 열람 가능. 현재 DB 데이터가 busan 중심이라 즉각 피해 없으나 진주·하동 운영 시 누수. currentUser.branch 기반 필터 추가 필요. | docs/AUDIT_SESSION31_FARM.md |
| FARM-TASK-SCOPE-001 | 데이터 스코프 | open | 세션 31 (2026-04-24) | - | taskStore.fetchTasks() / issueStore.fetchIssues() / attendanceStore.fetchRecords() branch 필터 없음 — farm_admin이 전 지점 데이터 열람 가능. 잠재적 스코프 누수. | docs/AUDIT_SESSION31_FARM.md |
| FARM-TASK-ADD-001 | 기능 미구현 | open | 세션 31 (2026-04-24) | - | TaskPlanPage "작업 추가" 버튼 없음 — /admin/tasks/new(TaskCreatePage) 라우트는 존재하나 진입 경로 없음. TopBar actions에 navigate('/admin/tasks/new') 버튼 추가 필요. | docs/AUDIT_SESSION31_FARM.md |
