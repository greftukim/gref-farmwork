...
| HQ-FINANCE-001 | 데이터 보완 | Phase1 완료·Phase2 대기 | 세션 22 (2026-04-22) 등록 → 세션 52 (2026-04-25) Phase1 완료 | - | finance_monthly + finance_budgets 신설(세션 52 마이그레이션). KPI 카드 3개(수확액/인건비/이익률) 실데이터 연결. filteredData Period useMemo 연결. Playwright PASS 39. 잔여: 차트(HQ-FINANCE-002), 입력UI(HQ-FINANCE-003). | docs/DESIGN_HQ_FINANCE_001.md |
| HQ-FINANCE-002 | 기능 미구현 | open | 세션 52 (2026-04-25) | - | HQFinanceScreen Phase 2 — 차트 3개 Recharts 실데이터 교체(월별 수확액/인건비 ComposedChart, 지점별 수익성 BarChart, 비용 구조 PieChart) + 예산 집행률 실연결 + FinanceTrendCard(DashboardInteractive.jsx:722) 실데이터 + Period 필터 완전 연결. kg당 원가 집계도 포함. HQ-DASHBOARD-INTERACTIVE-002 인건비 부분 해소 연계. | docs/DESIGN_HQ_FINANCE_001.md |
...
| HQ-DASHBOARD-INTERACTIVE-002 | 기능 미구현 | open | 세션 49 (2026-04-25) | - | DashboardInteractive 기간별 실집계 미완 — 가동률(오늘 이외 기간 attendance 집계), 인건비(재무 DB 없음), 일/주/분기 기간별 실수확 집계(현재 월×mult 근사). HQ-FINANCE-001 해소 후 연계 처리. | docs/BACKLOG.md |
...
| FARM-DASH-EXPORT-001 | 기능 미구현 | open | 세션 32 (2026-04-24) | - | AdminDashboard "내보내기" 버튼 — alert 임시 처리. 대시보드 데이터 CSV/PDF 내보내기 기능 구현 필요. 별 트랙. | docs/BACKLOG.md |
