...
| FLOOR-SCHEMA-SKIP-001 | 설계 메모 | open | 세션 19 (2026-04-22) → 세션 74-A 재분류 (2026-04-27) | - | [세션 74-A 정찰 수정] src/data/floor-schema.js 파일이 실존함. active 라우트(src/pages/FloorPlan.jsx)는 useFloorData 사용이 맞으나, dead code 파일(src/pages/admin/FloorPlan.jsx)이 이 mock을 import 중. dead code 파일 삭제 시 floor-schema.js도 함께 삭제 가능. wontfix 해제 → 74-B dead code 정리 시 해소 예정. | docs/HANDOVER_SESSION74A_RECON.md |
...
| MOBILE-FLOOR-001 | 기능 미구현 | open | 세션 73-B (2026-04-26) | - | MobileFloorScreen 평면도 뷰 — QR 위치 추적 기반 Tier 6 예정. 세션 73-C에서 attendanceStore 출근 인원(present/absent/total) + "준비 중" 안내 카드로 교체. MiniFloorPlan SVG + LegendItem 삭제 완료. PC-FLOOR-DATA-001 참조. | docs/BACKLOG.md |
...
| PC-FLOOR-DATA-001 | 기능 미구현 | open | 세션 73-C (2026-04-26) → 세션 74-A 설명 정정 (2026-04-27) | - | [세션 74-A 정찰 수정] active 라우트(src/pages/FloorPlan.jsx)는 useFloorData.js 기반. floor-schema.js를 사용하는 것은 dead code 파일(src/pages/admin/FloorPlan.jsx)뿐. 블로커: greenhouses/qr_codes/qr_scans 테이블 미존재 → DB 마이그레이션 시 PC 평면도 즉시 복원. timeAgo() 10:25 하드코딩 수정 필요(교훈 77). 74-B에서 해소 예정. MOBILE-FLOOR-001과 동일 트랙. | docs/HANDOVER_SESSION74A_RECON_2.md |
