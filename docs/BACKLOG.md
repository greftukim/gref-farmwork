...
| HQ-DASHBOARD-INTERACTIVE-002 | 기능 미구현 | partial | 세션 49 (2026-04-25) 등록 → 세션 53 (2026-04-25) 인건비 부분 해소 | - | DashboardInteractive 기간별 실집계 미완. FinanceTrendCard 인건비 실데이터 연결 완료(세션 53). 잔여: 가동률(오늘 이외 기간), 일/주/분기 기간별 실수확(월×mult 근사), 승인허브. 가동률은 attendance 집계 개선 필요. | docs/BACKLOG.md |
...
| STORE-MISSING-001 | BUG | open | 세션 55 (2026-04-26) | - | issueStore에 updateIssue 미존재 — IssueCallPage.jsx:29 `useIssueStore((s) => s.updateIssue)` undefined, 버튼 클릭 시 silent 무반응(optional chaining 보호). 해결: issueStore에 updateIssue(id, patch) 추가 또는 resolveIssue 재활용. 교훈 93 패턴. | docs/BACKLOG.md |
| STORE-MISSING-002 | BUG | open | 세션 55 (2026-04-26) | - | leaveStore에 approveRequest/rejectRequest 미존재 — LeavePage.jsx:17-18 undefined, 승인·반려 버튼 silent 무반응(optional chaining 보호). farmReview로 대체 가능 여부 검토 필요. 교훈 93 패턴. | docs/BACKLOG.md |
| STORE-MISSING-003 | BUG | open | 세션 55 (2026-04-26) | - | noticeStore에 markRead 미존재 — WorkerNoticePage.jsx:17 undefined, 공지 카드 클릭 시 읽음 처리 silent 무반응(optional chaining 보호). noticeStore에 markRead(id, userId) 추가 필요. 교훈 93 패턴. | docs/BACKLOG.md |
