...
## 교훈 126 — apply_migration 전 테이블 존재 여부 선행 확인 필수

DDL 마이그레이션 적용 전에 대상 테이블이 DB에 이미 존재하는지 반드시 확인해야 한다. `apply_migration`은 테이블이 이미 있으면 `42P07: relation already exists` 오류로 전체 트랜잭션이 실패한다.

**Why:**
세션 74-B에서 `greenhouses`, `qr_codes`, `qr_scans` 3개 테이블이 Supabase 대시보드에서 직접 생성돼있었음에도 마이그레이션 파일·RECON 보고서 어디에도 기록되지 않았다. `apply_migration` 호출 후 오류로 발견했다.

**How to apply:**
- `apply_migration` 전 항상 `SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name IN (...)` 쿼리 선행.
- 테이블 이미 존재 시: DDL 건너뛰고 차분(seed·constraint·RLS) 패치 전략으로 전환.
- 로컬 migrations/ 폴더에 파일이 없어도 Supabase 대시보드에서 직접 생성된 테이블이 있을 수 있다.

## 교훈 127 — 기존 테이블 스키마와 계획 차이 — 강제 재생성보다 현황 파악 후 차분 패치

이미 존재하는 테이블의 스키마가 계획과 다를 때, DROP + 재생성은 데이터 소실 위험이 있다. 실제 컬럼·제약·데이터를 조회한 뒤 누락된 항목만 추가(ALTER, INSERT)하는 패치 방식이 안전하다.

**Why:**
세션 74-B에서 `qr_codes` 테이블이 `label` 대신 `note`, `status`·`issued_at` 추가 컬럼을 갖고 있었고 FK가 RESTRICT 대신 CASCADE였다. 계획과 달랐지만 앱 코드가 의존하지 않는 컬럼이므로 무시했고, seed만 패치해 정상 동작.

**How to apply:**
- `information_schema.columns` → 컬럼 현황 확인
- `pg_constraint` → CHECK·FK 제약 현황 확인 (교훈 35 참조)
- `pg_policies` → RLS 정책 현황 확인
- 세 조회 후 계획과 차이 분석 → 앱 코드가 실제로 의존하는 컬럼/제약만 선별해 패치
- 앱이 사용하지 않는 컬럼(status, issued_at 등)은 현행 유지

## 교훈 128 — React Context 공유 시 Context 객체도 함께 export 해야 한다

`FloorCtx`처럼 내부 전용으로 선언된 Context를 외부 컴포넌트(MobileFloorScreen)에서 `Provider`로 사용하려면, Context 객체 자체를 named export해야 한다. export 없이 `GreenhousePlan`만 import하면 외부에서 `Provider` 래핑이 불가능하다.

**Why:**
세션 74-C에서 `GreenhousePlan`을 AdminMobilePages에 이식할 때, FloorCtx가 export되지 않아 `useContext(FloorCtx)` 호출이 빈 기본값을 반환할 뻔했다. export 전에 발견해 즉시 추가.

**How to apply:**
- 내부 Context를 외부에서 Provider로 사용하는 경우: `export { ..., FloorCtx }` 반드시 추가
- GreenhousePlan처럼 Context 의존 컴포넌트를 이식할 때: "이 컴포넌트가 어떤 Context를 `useContext`로 읽나?" 확인 → 해당 Context가 export돼 있는지 확인

## 교훈 129 — PDF에 한국어 텍스트: canvas 경유 이미지 방식으로 폰트 임베딩 없이 해결

jsPDF 내장 폰트(Helvetica 등)는 한국어를 지원하지 않는다. 별도 TTF 임베딩 없이 한국어를 PDF에 넣으려면 `<canvas>`에 브라우저 렌더링으로 텍스트를 그린 뒤 PNG 데이터 URL을 jsPDF에 삽입한다. 브라우저 캔버스는 OS 시스템 폰트를 사용하므로 한국어가 자연스럽게 렌더링된다.

**Why:**
세션 74-D에서 QR 라벨 PDF 생성 시 "1동 3골 앞" 같은 한국어 텍스트가 필요했다. jsPDF에 한국 폰트를 임베딩하면 수 MB 번들 증가. canvas 방식으로 추가 번들 없이 해결.

**How to apply:**
- `document.createElement('canvas')` → `ctx.font = '"Malgun Gothic","Apple SD Gothic Neo",sans-serif'` → `ctx.fillText(한국어, ...)` → `canvas.toDataURL('image/png')`
- `doc.addImage(dataUrl, 'PNG', x, y, w, h)`
- jspdf와 qrcode는 `dynamic import()`로 불러와 초기 번들에서 분리

## 교훈 130 — 결정권자 단일화: "박민식·김민국 답변 대기" 분류 영구 폐기

앱 수정 결정권자는 사용자 단일이다. 박민식·김민국은 현장 피드백 제공자이며 결정자가 아니다. 이후 핸드오버·BACKLOG에서 "박민식·김민국 답변 대기" 항목 발급 금지. 결정 보류가 필요하면 "사용자 결정 보류" 또는 "코드 판단 위임"으로만 표기.

**Why:**
세션 74-D 지시문에서 영구 원칙으로 명시됨. 현장 관리팀 피드백 → 사용자 경유 → 개발 반영 경로를 명확히 한다.

**How to apply:**
- BACKLOG 신규 항목 작성 시 담당자란에 "박민식/김민국 답변 대기" 입력 금지
- 기존 TEMP-DECISION-* 계열도 차기 세션 정리 시 "사용자 결정" 분류로 재기입

## 교훈 131 — null 누수 패턴 수정 시 동일 변수의 모든 사용처 grep 일괄 회수 필수

동일 변수(예: `task`, `worker`)에서 null 크래시가 발생해 특정 위치를 수정한 뒤, 같은 변수를 참조하는 다른 위치에 동일 패턴이 잔존할 수 있다. 1~2곳 발견·수정 후 "해결됨"으로 닫으면 크래시가 다른 경로에서 재현된다.

**Why:**
세션 74-F에서 `task.color` 크래시 2곳 수정(null-guard) 후 `resolved`로 닫았으나, 동일 `task` 변수를 쓰는 line 734(progress bar)·line 875(GolDetail 뱃지)에 같은 패턴 잔존. 세션 74-G에서 재발·재수정.

**How to apply:**
- 변수 null-guard 적용 후 반드시 `grep -n "변수명\." 파일` 일괄 실행
- 발견된 모든 `.속성` 접근 위치에 대해 null 가능성 점검 후 한 커밋으로 묶어 처리
- 특히 React 조건부 렌더(`{A && B}`) 안에서 A를 가드했더라도, 같은 변수를 쓰는 형제 블록은 별도 가드 필요

## 교훈 132 — 정찰 시 "기능 부재" 결론 전 페이지 전수 grep 의무화

페이지의 특정 기능이 "미구현"이라는 결론을 내릴 때, 한 영역(예: 카드 렌더, assignee 렌더 등)만 확인하고 다른 영역의 부재로 단정하면 이미 구현된 기능을 X→Y로 오분류한다.

**Why:**
세션 75-B Task 0 정찰에서 TaskBoardPage의 #5 우선순위 필터를 "기능 미구현(Y/S)"으로 분류했으나, 실제로는 line 43(state) + line 55(filter 조건) + line 96-111(칩 토글 UI) 모두 완전 구현되어 있었음. 정찰자가 line 167(assignees) 근처만 읽고 결론지은 결과. 75-B 진입 후 즉시 X케이스로 정정·BACKLOG 기록.

**How to apply:**
- 페이지의 "기능 부재" 결론 전 다음 3종 grep 의무 실행:
  1. `grep -n "useState\|state" {page}.jsx` — 관련 state 존재 여부
  2. `grep -n "filter\|Filter\|search" {page}.jsx` — 기존 필터 인프라 유무
  3. `grep -n "{기능 키워드}" {page}.jsx` — 직접 키워드 존재 여부
- 한 영역 확인 후 다른 영역의 부재로 단정 금지
- 파일 길이 대비 검토 비율 최소 70% 이상 확보 후 결론 도출
- 적용 범위: Task 0 정찰 보고서 작성 단계

## 교훈 133 — G33 안전 분기 패턴: 외부 의존 부재 시 write 비활성 + 활성화 마커

Supabase MCP 단절 등 외부 의존 부재 상황에서 write 작업이 필요한 트랙은 즉시 진행 시 운영 데이터 손상 위험. 그러나 일정 영향이 큰 경우 보류만으로는 부족.

**Why:**
세션 76-A-3a에서 모달 onSave 활성화는 MCP 검증이 선행돼야 했으나, 일정 영향과 다른 자산(WeekView·FocusList·View 토글)은 즉시 신설 가능했음. 단순 보류 시 트랙 진행 정지, 즉시 활성화 시 데이터 손상 위험.

**How to apply:**
3단계 분리 (UI 신설 + 검증 + 활성화)로 진행:
1. UI 신설 단계: write 핸들러는 `alert`만, 임시 비활성
2. UI 코드에 `BEGIN/END` 마커 주석으로 활성화 위치 명시 (예: `G33-MCP-DISCONNECT-GUARD: BEGIN`)
3. 외부 의존 복귀 후 검증 통과 시 마커 블록을 활성 코드로 1줄 변경 (micro-commit)
4. 마커 흔적은 해제 주석으로 보존 (코드베이스 검색 가능 — `G33-MCP-DISCONNECT-GUARD: 해제`)

**예시 트랙**: 76-A-3a (UI 신설 + onSave 비활성, `6cce107`) → 76-A-3b (사용자 검증 SQL 3건 통과) → 76-A-3c (1줄 변경 활성화, `3e78f32`)

## 교훈 134 — Supabase RLS UPDATE/SELECT + .single() latent bug

Supabase PostgREST에서 `.single()`은 정확히 1행 반환을 기대. RLS 정책으로 0행 영향(권한 차단·매칭 없음 등) 시 PostgREST는 `406 Not Acceptable` 반환 + Promise reject. UI에 error 분기가 없으면 사용자 시점 "무반응"으로 보임. 73-C 시점 코드가 그대로 보존됐음에도 권한 환경 변경 시 발현 가능 → 회귀로 오인.

**Why:**
QR 발급 hot-fix(`2939139`)에서 `employeeStore.issueDeviceToken`의 `.single()` + RLS UPDATE 0행 영향 → 406 + 사용자 무반응 보고. 73-C 시점부터 latent bug였으나 권한 분포에 따라 일부 사용자에게만 발현. QR 로그인 콜백(`b27519b`)에서도 authStore 3개 액션이 동일 패턴 사용해 silent failure.

**How to apply:**
- 0행 가능한 UPDATE/SELECT는 **`.maybeSingle()` 의무** 사용 (0행 시 null 반환, 406 안 발생)
- UI 호출처에서 `result.error` + `result.data === null` 분기 명시 처리
- 사용자 친화적 에러 메시지 (예: "권한이 부족합니다, 본인 지점의 작업자에게만 가능")
- RLS 정책 자체는 별 트랙 진단 (즉시 수정 X)
- 새 store 액션 작성 시 기본값으로 `.maybeSingle()` 채택

**예시 트랙**: QR 발급 hot-fix 옵션 3 (`2939139`), QR 로그인 콜백 (`b27519b` — authStore 3건 일괄 변경)

## 교훈 135 — PWA + Zustand persist + 다중 역할 인증 경쟁 조건

PWA 환경에서 `App.initialize()`의 `supabase.auth.getSession()` 비동기 조회와 다른 라우트(예: AuthCallbackPage)의 다른 역할 로그인이 동시 발생 시, set 순서에 따라 한쪽이 다른 쪽을 silent 덮어씀. React 상태는 Promise 사이클 사이 race가 발생해도 단순 set 순서대로 적용. 결과: ProtectedRoute가 잘못된 역할 검사 → 의도 외 redirect.

**Why:**
QR 로그인 hot-fix(`b27519b`)에서 사용자 보고 "QR 찍어서 모바일로 작업자계정 진입했는데 계속 로그인 화면이 뜬다". 정찰 결과: AuthCallbackPage의 `loginWithToken`이 worker 세션 set한 직후, App.initialize가 비동기로 끝나며 잔존 관리자 세션을 set → ProtectedRoute가 worker 권한 체크 실패 → /login redirect.

**How to apply:**
다른 역할 로그인 진입점은 다음 두 단계로 직렬화:
1. 기존 세션 명시 정리: `await supabase.auth.signOut()` 또는 store의 currentUser 초기화
2. 새 로그인을 `await`로 직렬화 (`.then()` 사용 금지, async/await + try/catch 의무)

또한:
- AuthCallbackPage 등 인증 진입점은 `.then()` 사용 금지 (uncaught Promise reject 방지)
- HydrationGate가 hydration 완료 전 ProtectedRoute 렌더 차단되는지 확인
- Zustand persist가 다른 역할 세션을 캐시할 가능성 인지 (partialize 신중)

**예시 트랙**: QR 로그인 콜백 hot-fix 분기 H (`b27519b`)

## 교훈 136 — 사용자 답변으로 정찰 단축 위험

운영 의도·사양 결함 가능성 사항도 사용자 답변(추정·기억)으로 정찰을 단축하면 코드 사실과 어긋날 위험. 본 채팅방 가설이 코드 실제와 정반대 방향으로 진행될 수 있음.

**Why:**
QR 버튼 hr_admin 권한 확장 트랙에서 사용자 보고 "박민식·김지현 hr_admin에게 QR 버튼 미표시" + 사용자 가설 "EmployeesPage 권한 분기에 hr_admin 누락". 그러나 코드 정찰 결과 EmployeesPage에는 currentUser 권한 분기 자체가 0건 (단일 `e.role === 'worker'`만). 진짜 원인은 AdminLayout 라우팅 분기로 HQ 사용자가 별도 화면(HQEmployeesScreen)으로 라우팅되며 거기에 QR 발급 코드 0건. 사용자 가설 채택 시 코드 변경해도 문제 해결 안 됨 → 가설 폐기 후 G62 옵션 A(이식) 채택 (`57da63f`).

**How to apply:**
1. 사양 결함으로 보이는 사항도 **코드 정찰 우선** (grep + git diff + DB 사실 확인)
2. 사용자 답변은 정찰 결과와 교차 검증
3. 정찰 단축 결정 전 본 채팅방의 가설 일관성 확보
4. 정찰 결과로 가설 반증 시 즉시 가설 폐기 후 재계획 (지시문 §10 정지 조건 적극 활용)
5. "사용자 추정"과 "코드 사실"을 보고서에 명시 분리

**예시 트랙**: QR 버튼 hr_admin 가설 → HQEmployeesScreen 별 화면 라우팅 발견 → 가설 폐기 (`57da63f`)

## 교훈 137 — 정찰 깊이: 시그니처 불일치는 표면 grep으로 안 보임

시나리오 X/Y(인프라 존재 / 통합 누락만) 판정 시 "함수 존재 + DB 시드 존재"만으로는 부족. 함수 시그니처가 정책 요구와 일치하는지, 데이터 흐름이 운영 데이터와 정합한지까지 검증해야 함. 표면 grep으로는 인프라 본질 결함이 안 보임.

**Why:**
출퇴근 검증 hot-fix v1 정찰에서 "useGpsVerify·judgeAttendanceStatus·DB 시드 모두 존재" 확인 → 시나리오 Y(통합 누락만) 판정. v1 hot-fix 진입 후 발견:
- `judgeAttendanceStatus(checkInTime, workStartTime)` 2 인자만 받음, 임계값 인자 X → 정책 G67(5분 후 지각) 적용 불가
- `locationStore`는 hardcoded 단일 좌표(35.0956/128.9746/200m)이며 운영 시드(35.08755/128.959102/500m)와 약 1.6km 차이 + branches 데이터 무시 → 다지점 운영 부적합

→ v1 정지 후 G71 옵션 (나) 채택, WorkerHome 인프라 우회로 v2 통합 (`6a4e884`).

**How to apply:**
시나리오 X/Y 판정 시 다음 3단계 검증 의무:
1. **인프라 존재 여부** (grep)
2. **시그니처가 요구사항과 일치** (인자 목록·반환값·임계값·정책 옵션 등)
3. **데이터 흐름이 운영 데이터와 정합** (hardcoded vs 운영 시드, fetch 대상 vs RLS, persist 캐시 등)

3단계 모두 통과해야 시나리오 X/Y 확정. 한 단계 미통과 시 시나리오 부분 폐기 + 재계획.

대안 제시 시:
- 옵션 (가) 인프라 보강 트랙으로 격상 (정확하나 일정 +0.5~1일)
- 옵션 (나) 호출처에서 인프라 우회 (빠르나 일관성 깨짐, 부채 BACKLOG 등록 필수)
- 옵션 (다) 본 트랙 분리 + 일부만 즉시 처리

**예시 트랙**: 출퇴근 검증 hot-fix v1 정지 → v2 옵션 (나) 채택 (`6a4e884`) + 부채 LOCATION-STORE-MULTI-BRANCH-001 / JUDGE-ATTENDANCE-THRESHOLD-001 등록

## 교훈 138 — git push 누락 → 사용자가 시각 변화 못 봄 (배포 검증 단계 필수화)

라운드 종료 시 로컬 commit만으로 끝내면 Vercel 배포본은 그대로. 사용자가 "변화 없음"이라 보고 → 디버그 시간 낭비. 라운드 종료 단계에 git push + 원격 검증 + Vercel 트리거 안내까지 1세트로 묶어야 함.

**Why:**
트랙 77 U0~U5 6라운드 모두 로컬 커밋 완료 후 push 누락. 사용자가 화면 변화 없음 신고 → git log 확인 후 발견. origin/main이 `990d4db`(트랙 76-A 종료 시점)에 머물러 있었음. 1회 `git push origin main`으로 7개 커밋(`990d4db..2cd68c5`) 일괄 전송 + Vercel webhook 자동 발화로 즉시 배포됨.

**How to apply:**
라운드 종료 단계에 다음 1세트 강제:
1. 로컬 commit 완료 후 즉시 `git push origin main`
2. `git log -1 --format="%H" origin/main`으로 원격 HEAD 매칭 확인
3. 보고서 §6에 다음 박제: 로컬 SHA / push 결과 / Vercel 트리거 안내
4. 사용자에게 "Vercel 배포 Ready 후 강제 새로고침 또는 PWA 재설치" 안내

**예시 트랙**: 트랙 77 U0~U5 일괄 push (`990d4db..2cd68c5`) + U6~U10 라운드별 push 흐름 박제

## 교훈 139 — 시안 메타 주석 vs 실제 코드 매칭 검증 (시각만 적용 회귀)

디자인 시안에 "useEffect 마운트 시 카메라 자동 시작 (Q6) → permissionStatus 분기"라 명시되어도, 실제 코드에 useEffect 호출이 있다고 끝나는 게 아님. **레이아웃 타이밍/조건부 렌더링 부작용까지 검증**해야 함. 시안 메타 주석은 의도이고, 실제 동작은 별도.

**Why:**
트랙 77 U2에서 QrScanPage `qr-reader` div가 `display: scanning ? 'block' : 'none'` 조건부 렌더. 마운트 시점 useEffect → startScan() 호출 → html5-qrcode가 `display:none` 부모에 video 엘리먼트를 0×0으로 부착 → 이후 `setScanning(true)` 후에도 video는 0×0 고정. 사용자가 U6에서 "카메라 영상 안 보임" 신고. U2 시안 메타 주석은 "자동 시작" 명시했고 코드도 그렇게 되어있었으나 **레이아웃 타이밍 부작용으로 실제 동작 안 함**.

**How to apply:**
시각 시안 적용 라운드 종료 시 검증:
1. **인터랙션 흐름 정적 분석** (onClick / useEffect / 함수 호출 매칭)
2. **렌더링 부작용 검토** (display:none 시점에 외부 라이브러리가 dimension 의존하는지)
3. **사용자 환경 검증** (개발 중인 PC 빌드 + Vercel 배포본 + 실 PWA 모두 확인)
4. 라이브러리(html5-qrcode 등) 사용 시 부모 컨테이너 dimensions 명시적 보장

3단계 모두 통과해야 시각 적용 라운드 클로저.

**예시 트랙**: 트랙 77 U2 → U6 fix (`b408d6b`)

## 교훈 140 — latent bug 동일 패턴 grep 일괄 회수 (s.user → s.currentUser 이중 회귀)

null 누수/undefined 누수/오타 패턴 발견 시 같은 패턴이 다른 파일에도 있을 가능성. 한 군데 fix만으로 끝내지 말고 즉시 동일 패턴 grep으로 모든 사용처 일괄 회수해야 함. 교훈 131 강화판.

**Why:**
트랙 77 U4에서 WorkerNoticePage가 `useAuthStore((s) => s.user)` 사용 발견 → `s.currentUser`로 fix (authStore는 `currentUser`만 export). U8 전체 검수 단계에서 WorkerLeavePage에 동일한 `s.user` 패턴 발견. U4 시점에 grep으로 일괄 회수했으면 U8까지 회귀가 안 남음. 임팩트: WorkerLeavePage는 휴가 신청 시 `employeeId: undefined` → DB INSERT NOT NULL 위반 또는 NULL 삽입.

**How to apply:**
latent bug fix 시 의무 절차:
1. 발견한 패턴을 정확하게 정의 (예: `useAuthStore((s) => s.user)`)
2. `grep -rn "s\.user" src/` 등으로 동일 패턴 모든 사용처 회수
3. 각 사용처별 fix 또는 의도적 보존 판단
4. fix된 파일 목록 + 보존 결정 사용처를 보고서에 박제

본 절차는 교훈 131의 "변수 사용처 grep 일괄 회수"와 동일하나, **더 명시적으로 `한 번에 다 fix or 다 보존`으로 완결**시켜야 함.

**예시 트랙**: 트랙 77 U4 (WorkerNoticePage) + U8 (WorkerLeavePage 회귀 발견 + fix `b27094a`)

## 교훈 141 — 자율 시행 + 사후 보고 흐름의 균형점 (결정 게이트 자율화)

복잡한 다중 라운드 작업에서 모든 결정 게이트마다 사용자 답변 대기 시 진행 속도 ↓ + 사용자 피로 ↑. 반대로 모든 결정을 자율 시행 시 통제 ↓ + 회귀 위험 ↑. **균형점**: 자산 보존 위반 가능성 + DB 변경 + 외부 인프라 변경만 사전 질문, 그 외는 자율 + 사후 보고.

**Why:**
트랙 77 G77-A는 사용자 사전 답변(reload), 이후 G77-B~I는 사용자 흐름 변경 지시로 자율 결정 + 보고서 박제로 전환. 결과: 라운드별 진입 속도 향상 + 사용자 의사결정 부담 감소. 단, G77-H Storage 활성화처럼 DB 변경 항목은 자율 시행 예외 → BACKLOG 박제 + 별 트랙 권고로 처리.

**How to apply:**
다중 라운드 작업 시 결정 게이트 분류:
- **사전 질문 필수**: 자산 보존 위반 가능성 / DB 스키마 변경 / 외부 인프라 변경 (Storage, env vars, RLS) / 200+ 라인 신규 / 사용자 멘탈 모델 직접 영향
- **자율 + 사후 보고**: 시각/UX 결정 / 코드 패턴 선택 / 라이브러리 vs 정적 데이터 / 컴포넌트 분리 vs 인라인

자율 결정 시 보고서 §4에 다음 박제 의무:
1. 결정 ID
2. 선택 옵션
3. 권고 근거
4. 대안 (다른 옵션이 있었다면 명시)
...
**예시 트랙**: 트랙 77 G77-A (사용자 확정) → G77-B~I (자율) + Storage·notice_reads 별 트랙 박제

## 교훈 142 — Sub-Agent 회귀 검수 효과 (정적 분석 위임 패턴)

다중 파일 변경 라운드 종료 시 회귀 위험 점검을 메인 컨텍스트에서 직접 수행하면 컨텍스트 윈도우 소진 + 검수 깊이 ↓. **Sub-Agent에 정적 분석 위임** + 결과 보고만 메인으로 회귀 → 검수 깊이 + 컨텍스트 효율 동시 확보.

**Why:**
트랙 77 U8 작업 C-1 전체 검수에서 Sub-Agent 위임으로 12개 파일 정적 분석 + onClick/useEffect/데이터 흐름/라우트 정합성 일괄 점검. P0 위반 1건(WorkerLeavePage `s.user`) + P2 경고 1건(WorkerTasksPage 상세 버튼) 발견. 메인 컨텍스트에서 직접 수행 시 12개 파일 read + 분석 + 보고서 = 컨텍스트 부담 큼. Sub-Agent 위임 시 1회 호출 + 500단어 보고서로 압축.

**How to apply:**
다중 파일 변경 라운드 종료 시 다음 패턴 적용:
1. 메인 컨텍스트는 변경 의도 + 자가 검증만 수행
2. Sub-Agent에 검수 항목 명세 위임 (검수 대상 파일 / 위반 패턴 / 출력 형식 명시)
3. Sub-Agent 결과를 메인이 받아 P0/P2 분류 + 즉시 fix
4. 보고서에 검수 결과 + Sub-Agent 위임 사실 박제

검수 항목 명세 시 포함 (트랙 77 U8 패턴):
- A. onClick 핸들러 실효성
- B. useEffect 실효성
- C. 데이터 흐름 검증
- D. 시각 외 누락
...
**예시 트랙**: 트랙 77 U8 검수 (Sub-Agent 위임 → P0 1건 + P2 1건 발견 → 즉시 fix `b27094a`)

## 교훈 143 — byte-identical 보존 패턴 (자산 보존 7건 안전 마진)

핵심 자산(예: 출퇴근 v2 인라인 함수)의 변경 금지를 강제하는 가장 강력한 패턴은 **byte-identical 검증**. Sub-Agent로 baseline 커밋과 현재 커밋의 함수 본문을 직접 diff. 단 1줄이라도 변경이 있으면 위반.

**Why:**
트랙 77 6라운드(U1~U5) + 후속 라운드(U6~U9) 모두 출퇴근 v2 인라인 함수(haversineDistance / getCurrentPosition / timeStrToTodayDate / verifyBranchAndGps / handlePunch)에 대해 byte-identical 검증 수행. 시각 재설계가 큰 라운드(U1)에서도 함수 본문은 0줄 변경. 사용자가 운영 중인 출퇴근 검증 hot-fix v2(`6a4e884` `ATTENDANCE-VALIDATION-HOTFIX-002`)가 손상 없이 그대로 작동.

**How to apply:**
자산 보존 항목별 검증 등급 차등:
1. **byte-identical 검증** — 핵심 인프라 함수 (출퇴근 v2, GPS 검증, QR INSERT 흐름 등). Sub-Agent로 `git show <baseline>:<file> | grep -A N <function>` 후 현재 파일과 본문 비교
2. **시그니처 보존 검증** — 외부 인터페이스 (store API, prop 타입). 시그니처만 일치하면 본문 리팩토링 OK
3. **DB 스키마 보존** — 신규 컬럼/테이블 추가 0건 + 기존 컬럼 타입/제약 변경 0건
...
## 교훈 151 — 데이터 라이프사이클 분리 원칙 (영구 자산 vs 회차 자산)

같은 도메인 객체라도 **영구 자산**(물리 구조·메타데이터)과 **회차 자산**(작기·이벤트·시즌)은 **테이블 분리**가 정답이다. 같은 폼/테이블에 묶으면 (1) 회차 변경마다 영구 데이터 재입력 위험, (2) 시계열 누적 데이터(AI 학습) 매트릭스가 흐려진다, (3) 컬럼 일부 NULL이 늘어 의미 추적 ↓.

**Why:**
트랙 77 U20에서 사용자 통찰: "온실 물리 구조는 계속 동일하고 식재 정보는 작기 시작할때마다 달라질건데 분리를 해야 하지 않을까?". 초안 v1은 단일 zone_meta 테이블에 모두 묶었으나 사용자가 v3로 분리 제안 — 운영 채팅방 합의 후 채택.

해결: `zone_specs` (영구 — 베이 길이/폭/통로) + `zone_crops` (회차 — 작물·품종·식재정보·시작·종료) + `zone_crop_events` (회차 내 자유 시점). FK 체인으로 무결성 + 시계열 누적 매트릭스 보장.

**How to apply:**
신규 도메인 객체 설계 시 의무 절차:
1. 객체 속성을 라이프사이클 축으로 분류 — 영구(평생 1회) / 회차(시즌·작기·연단위) / 이벤트(자유 시점)
2. 라이프사이클별 테이블 분리:
   - 영구 자산 = PK가 부모 ID (1:1)
   - 회차 자산 = PK가 자체 UUID + FK가 부모 ID (1:N)
   - 이벤트 = PK가 자체 UUID + FK가 회차 자산 ID (1:N)
3. 폼 UI도 라이프사이클별 분리 (탭 또는 모달 분리)
4. AI 학습 누적 의도 박제 — 회차 데이터는 절대 UPDATE 덮어쓰기 금지, 새 row INSERT 패턴
5. ended_at 컬럼으로 회차 종료 표현 (DELETE 금지 — 시계열 누적 손실)

**예시 트랙**: 트랙 77 U20 (`zone_specs` 1:1 영구 + `zone_crops` 1:N 회차 + `zone_crop_events` 1:N 자유 시점 — AI 학습 데이터 누적 인프라 박제)

---

## 교훈 152 — 엑셀 산식 → 클라이언트 derive 전략

엑셀 또는 외부 도구의 계산 산식을 앱에 이식할 때, **입력값만 DB 저장 + 계산값은 클라이언트 derive**가 정답이다. (1) 산식 변경 시 DB 마이그레이션 불필요, (2) 모든 화면에서 일관된 계산, (3) 사용자에게 산식 박제 footer 표시로 신뢰 ↑. **단계 분리**도 중요 — 큰 산식 4단계 중 사용자 우선순위 기반 P0+P1만 우선 구현, P2~P3은 별 트랙.

**Why:**
트랙 77 U20에서 사용자 제공 엑셀(암면 계산기) 분석 결과 4단계 산식 (① 온실 물리 구조 ② 식재 + 재식밀도 ③ 자재 소요량 ④ 발주 가격). 본 라운드 ①+②만 구현, ③④는 별 트랙. 운영 데이터가 누적된 후 우선순위 재평가하기 위함.

해결: `src/lib/zoneCalc.js` 신규 — `calculateZoneArea(spec)` + `calculatePlantingDensity(zoneCrop, zoneArea)` + `FORMULA_NOTES` 산식 박제 export. 컴포넌트(`DensityCalcTab.jsx`)에서 useMemo로 derive + footer에 산식 표시.

**How to apply:**
엑셀 산식 이식 시 의무 절차:
1. 산식을 단계별로 분해 (P0/P1/P2/...) — 각 단계 입력→출력 명확히
2. 입력값 = DB 컬럼 / 계산값 = 클라이언트 derive (DB에 절대 저장 금지)
3. 산식 헬퍼 = 단일 파일 (`src/lib/<도메인>Calc.js`) + 입력 객체 받아 출력 객체 반환
4. 산식 박제 = `FORMULA_NOTES` 객체로 export → 컴포넌트 footer에 표시
5. 단계 분리 = 우선순위 기반 P0/P1 먼저, 나머지는 별 트랙 BACKLOG
