...
**최종 갱신:** 2026-04-25 (세션 52) — Gate 5건 결정 반영  
**작성자:** Claude  
**상태:** Gate 결정 완료 · Phase 1 구현 중 (세션 52)

## Gate 결정 (태우님, 세션 52)

| Gate | 결정 | 비고 |
|------|------|------|
| F-01 매출 소스 | **A안 (수동 입력)** | 미래 C안 전환 가능 (crop_unit_prices + RLS 컬럼 추가) |
| F-02 인건비 소스 | **A안 (수동 입력)** | DB 임금 데이터 부재로 사실상 A 외 선택 없음 |
| F-03 farm_admin 입력 | **A안 (hr_admin 전담)** | Phase 3 재검토 — 운영 구조 명확화 후 B/C 전환 검토 |
| F-04 금액 단위 | **A안 (원 단위 DB 저장)** | 회계 표준. UI 변환: `(v / 10000).toFixed(1) + '만원'` |
| F-05 예산 수정 UI | **A안 (시드 고정)** | Phase 3 재검토 — 연간 변경 빈도 낮음 |
