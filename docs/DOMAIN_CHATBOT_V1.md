...
### 1.2 사용자 대상 (v2 변경)
**farm_admin + hr_admin + master + general만 사용**. worker·team_leader는 챗봇 미노출.
...
[Edge Function: chatbot-query]
       ↓ JWT 검증
       ↓ user_role이 farm_admin/hr_admin/master/general 아니면 403 거부
...
v1 챗봇 제약:
- 챗봇은 admin 전용(farm_admin/hr_admin/master/general). worker·team_leader는 접근 불가.
...
- **노출 role**: farm_admin / hr_admin / master / general 4종. supervisor 제외 (Edge Function ALLOWED_ROLES 4종과 노출 조건 일치).
...
| **H-3** | AdminLayout FAB(z-[60]) + 슬라이드 오버 패널(z-[70], 모바일 전체화면/데스크톱 440px). useChatStore(Zustand non-persist). supabase.functions.invoke 연동. role 4종(farm_admin/hr_admin/master/general) 노출, supervisor 제외. 13pt Pretendard(전역 로드 완료). D1~D7 UX 결정 §5 참조. 작업 단위 6개 (단위 1~6). 완료 (2026-04-14, 세션 15 단위 6 마감). J-3(2026-04-15, 세션 16)에서 general 추가. |
